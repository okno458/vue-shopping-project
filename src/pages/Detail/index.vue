...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //表单元素修改产品个数
      changeSkuNum(event){
        //使用户输入的文本*1 若用户输入的文本不为数字，则*1的结果一定是NaN
        let value = event.target.value * 1;
        //如果输入的文本非法，如NaN或者输入的文本为负数
      ...
      //加入购物车
      async addShopCar(){
        //1.在点击加入购物车按钮时将参数带给服务器发请求，将产品加入到数据库
        try {
          //2.需要知道这次请求是否成功，若成功进行路由跳转，若失败则给用户进行提示
          await this.$store.dispatch('AddOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum});
          //3.进行路由跳转
          //4.将产品信息带给下一级的路由组件，简单的数据skunum可以通过query传递
          //但产品信息比较复杂，通过会话存储(不持久化，会话结束就会消失)
          //本地存储或会话存储一般都存的是字符串
