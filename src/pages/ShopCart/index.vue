...
import throttle from 'lodash/throttle'
  export default {
    ...
    methods: {
      //获取个人购物车的数据
      getData(){
        //发请求时，发现获取不到购物车中的数据，因为服务器无法验证用户的身份，需要uuid来验证身份
        this.$store.dispatch('getCartList');
      },
      //修改产品的个数（节流）
      handler:throttle(async function(type,disNum,cart){
        //type用于区分元素
        //disNum形参 用于传递变化量和input最终的个数
        //cart用于区分点击哪一个产品
        switch (type) {
          //加号
          case 'add':
            ...
            break;
          case 'minus':
            //判断产品的个数，只有大于1才传递给服务器-1,如果产品的个数小于等于1，则给服务器传递0
            disNum = cart.skuNum > 1 ? -1 : 0;
            break;
          case 'change':
            //用户输入进来的量是非法的(带有汉字，负数)
      ...
      async updateChecked(cart,event) {
        //判断复选框是否选中，若选中则给服务器传递1，未选中则传递0
        let isChecked = event.target.checked ? '1' : '0';
        try {
          await this.$store.dispatch('UpdateCheckedBId',{skuId:cart.skuId,isChecked});
        } catch (error) {
